...
const AdminLoginPage = ({ onSuccess }) => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#060408", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#120608,#0d0604)", border: `2px solid ${error ? "rgba(255,50,50,0.4)" : "rgba(255,69,0,0.25)"}`, borderRadius: "24px", padding: "50px 44px", width: "100%", maxWidth: "420px", textAlign: "center", boxShadow: `0 0 80px ${error ? "rgba(255,50,50,0.15)" : "rgba(255,69,0,0.12)"}`, animation: shake ? "shake 0.4s ease" : "fadeIn 0.5s ease" }}>
        <div style={{ marginBottom: "8px" }}>
          <div style={{ background: C.gradient, borderRadius: "12px", padding: "8px 16px", fontFamily: "'Bebas Neue', cursive", fontSize: "28px", color: "#fff", letterSpacing: "3px", display: "inline-block" }}>🏏 CRICIFY</div>
        </div>
        ...
        <input type="password" value={pass} onChange={e => { setPass(e.target.value); setError(false); }} onKeyDown={e => e.key === "Enter" && tryLogin()} placeholder="Password darj karein..." autoFocus
        ...
        <button onClick={tryLogin} style={{ width: "100%", background: C.gradient, border: "none", color: "#fff", borderRadius: "12px", padding: "14px", cursor: "pointer", fontWeight: "800", fontSize: "16px", marginBottom: "20px" }}>🔓 LOGIN</button>
        <div style={{ color: "#2a1a10", fontSize: "11px" }}>© Cricify Admin Panel</div>
      </div>
    </div>
  );
};
        ...
        <div style={{ width: "100%", aspectRatio: "16/9", background: "#0a0200", position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
          {match.stream_url ? (
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ color: C.accent, fontSize: "20px", fontWeight: "800", fontFamily: "'Bebas Neue', cursive" }}>STREAM COMING SOON</div>
            </div>
          )}
          <div style={{ position: "absolute", top: "16px", left: "16px", background: "rgba(0,0,0,0.88)", borderRadius: "10px", padding: "10px 16px", border: `1px solid ${C.accentBorder}`, zIndex: 10 }}>
            <div style={{ display: "flex", gap: "20px", alignItems: "center" }}>
              <div style={{ textAlign: "center" }}><div style={{ color: "#aaa", fontSize: "10px" }}>{match.team1}</div><div style={{ color: "#fff", fontSize: "18px", fontWeight: "800", fontFamily: "'Bebas Neue', cursive" }}>{match.score1}</div></div>
              <div style={{ color: C.accent, fontWeight: "900" }}>VS</div>
              <div style={{ textAlign: "center" }}><div style={{ color: "#aaa", fontSize: "10px" }}>{match.team2}</div><div style={{ color: "#fff", fontSize: "18px", fontWeight: "800", fontFamily: "'Bebas Neue', cursive" }}>{match.score2}</div></div>
            </div>
          </div>
        </div>
        <div style={{ padding: "14px 20px", background: "#120600", display: "flex", alignItems: "center", gap: "16px", borderTop: `1px solid ${C.borderMid}` }}>
          ...
          <div style={{ display: "flex", gap: "6px" }}>
            {["480p","720p","HD","4K"].map(q => (
              <button key={q} onClick={() => setQuality(q)} style={{ background: quality === q ? C.gradient : C.accentSoft, color: quality === q ? "#fff" : C.accentBright, border: `1px solid ${C.accentBorder}`, borderRadius: "6px", padding: "4px 10px", fontSize: "11px", cursor: "pointer" }}>{q}</button>
            ))}
          </div>
        </div>
      </div>
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Segoe UI', sans-serif", color: C.textPrimary }}>
      ...
      <main style={{ maxWidth: "1200px", margin: "0 auto", padding: "24px 20px", display: "flex", gap: "24px" }}>
        <div style={{ flex: 1 }}>
          ...
          {loading ? (
            <div style={{ textAlign: "center", padding: "80px", color: C.accentBright }}><div style={{ fontSize: "40px", marginBottom: "16px" }}>⏳</div>Loading...</div>
